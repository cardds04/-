 ...
 * NAVER_WORKS_DRIVE_PARENT_FILE_ID(공용 상위)·네이버웍스 API(JWT 또는 access token). Vercel에서는 Node HTTP, 로컬은 Python(create_folder.py) 가능.
 */
...
function runCreateFolderPyScript(folderName, parentFileId) {
  ...
  const nodeOut = tryNaverWorksFolderNodeWorker({ folderName: name, parentFileId: pid, reuseIfExists: false });
  if (nodeOut) return nodeOut;
  ...
  if (r.error) {
    ...
    if (code === "ENOENT") {
      const fb = tryNaverWorksFolderNodeWorker({ folderName: name, parentFileId: pid, reuseIfExists: false }, {
        force: true,
      });
      if (fb) return fb;
