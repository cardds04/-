...
async function wsCreate(key, body) {
  ...
  if (mode === "kling") {
    ...
    // 🎬 클링 — 인물 전체(얼굴+머리+복장)를 캐릭터 이미지로 교체. prompt로 '전부 교체'를 명시.
    payload = { image: face, video, character_orientation: (body.character_orientation === "image" ? "image" : "video"), keep_original_sound: body.keep_original_sound !== false, prompt: String(body.prompt || "Replace the person in the video entirely with the given character — keep the character's exact face, full hairstyle, and clothing/outfit. The output person must wear the character's outfit and hairstyle, not the original video's.") };
  } else if (mode === "head") {
