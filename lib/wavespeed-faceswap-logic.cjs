...
const WS_MODEL = "wavespeed-ai/video-face-swap";
...
async function wsCreate(key, body) {
  ...
  const payload = { video, face_image: face, target_index: Number.isFinite(+body.target_index) ? +body.target_index : 0 };
  const r = await fetch(`${WS_BASE}/${WS_MODEL}`, {
    method: "POST",
