...
// mode: "face"=얼굴 부위만(머리·안경 유지) / "head"=머리 통째로(머리카락·안경까지 교체)
const WS_MODELS = { face: "wavespeed-ai/video-face-swap", head: "wavespeed-ai/video-head-swap" };
...
async function wsCreate(key, body) {
  ...
  const mode = body.mode === "head" ? "head" : "face";
  const model = WS_MODELS[mode];
  const payload = mode === "head"
    ? { video, face_image: face }   // 머리 교체 — target_index 없음. 해상도 기본(서버 기본값)
    : { video, face_image: face, target_index: Number.isFinite(+body.target_index) ? +body.target_index : 0 };
