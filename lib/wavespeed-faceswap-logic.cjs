...
async function wsCreate(key, body) {
  ...
  if (mode === "kling") {
    ...
    payload = { image: face, video, character_orientation: (body.character_orientation === "image" ? "image" : "video"), keep_original_sound: body.keep_original_sound !== false };   // 방향: video=움직임 영상 기준
  } else if (mode === "head") {
