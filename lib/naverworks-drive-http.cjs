  ...
  if (directToken) {
    ...
    if (expAtStr && refreshTok) {
      ...
      if (Number.isFinite(expSec) && Date.now() / 1000 >= expSec - 30) {
        ...
        if (!rr.ok || !j.access_token) {
          // ‼️refresh 토큰까지 만료된 상태(2026-08-07 실사고: 모든 폴더 생성 502).
          //   서비스어카운트 JWT 로 새 토큰을 발급받아 자가복구한다.
          //   (Vercel 에 PRIVATE_KEY·SERVICE_ACCOUNT 환경변수 필요)
          try {
            return await jwtFallback();
          } catch (e) {
            throw new Error(
              `토큰 refresh 실패(Naver Works) + JWT 폴백 실패: ${e?.message || e}`
            );
          }
        }
