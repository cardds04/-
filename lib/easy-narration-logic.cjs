...
const DIARY_TONE_SYSTEM = `너는 숏폼(릴스/쇼츠) 영상에 깔리는 한국어 '독백 자막'을 쓴다.
- ⭐가장 중요: [말투 예시]에 나온 사람의 '말투'를 그대로 흉내낸다. 어미·어투·존댓말이냐 반말이냐·자주 쓰는 표현·말버릇·문장 리듬·감탄사까지 똑같이 따라한다. 예시의 '내용'은 절대 베끼지 말고 '말투'만 가져온다.
- [주제]에 대해, 일기 쓰듯·혼잣말하듯 1인칭 시점으로 쓴다.
- 담담하고 진솔하게, 보는 사람이 공감하도록.
- 한 편의 이어지는 글(여러 문장). 해시태그·이모지 남발 금지.
- 출력은 독백 대본 글만. 설명·머리말·따옴표·번호·길이표기 금지.
...
async function genDiary(body) {
  ...
  let user, system;
  if (tone) {
    user = `다음 [말투 예시]의 말투를 그대로 흉내내서, [주제]로 영상에 깔 독백 자막을 써줘. 공백 포함 한국어 약 ${maxChars}자(±15%) 분량. ⭐말투 예시의 어미·어투(존댓말/반말)·말버릇·표현을 똑같이 따라하되, 예시 '내용'은 베끼지 말고 '주제'에 대해 새로 써.\n\n[말투 예시]\n${tone}\n\n[주제]\n${prompt}`;
