...
async function genDiary(body) {
  ...
  const user = `다음 주제로 영상에 깔 독백 자막을 써줘. 일기 쓰듯 말하듯 독백체로, 공백 포함 한국어 약 ${maxChars}자(±15%) 분량으로. ⭐반드시 존댓말(해요체)로, 반말 쓰지 마. "~했는데요/~이에요/~더라고요" 같은 부드러운 존댓말 어미로.\n\n[주제]\n${prompt}`;
  const text = await callClaude(user, Math.min(2200, maxChars * 4 + 300), { system: DIARY_SYSTEM, temperature: 0.92 });
