...
// ── 후킹 스타일(손님이 칩으로 선택) — 각 스타일별 추가 지시 ──
const HOOK_STYLES = {
  scene: { extra: "" },   // 기본(장면형) — 시스템 프롬프트 그대로
  news: { extra: `
[이번엔 '낚시성 뉴스 기사 제목' 스타일로 — 이게 이번 핵심]
연예·가십 기사 제목처럼, 사실의 한 조각만 떼어 자극적으로 재포장해 '낚아라'.
- 핵심 기법: 진짜 사실인데 맥락을 쏙 빼서 스캔들·충격처럼 보이게. 단, 영상에서 진짜 맥락(대개 별것 아니거나 오히려 좋은 일)이 밝혀지는 '사실 기반'이어야 한다 — 새빨간 거짓은 금지.
  (예: 배우가 드라마에서 이혼 연기 → 기사 제목 "OO 결혼 3개월 만에 파경"처럼, 한 조각만 떼어 충격 제목으로.)
- 이 업종 버전 결: "[단독] 30년 단골, 결국 발길 끊었다" / "충격…사장님 갑자기 가게 내놨다" / "OO, 손님들 줄줄이 환불 요청" (→ 실제론 이사·확장이전·리뉴얼·너무 만족 등 반전)
- 기사체 어휘를 적극 활용: [단독] [충격] [속보] '결국' '한 달 만에' '발칵' '비상' '줄줄이' '전말' '알고 보니' '경악'.
- 제목 한 줄만. 따옴표·이모지 없이.` },
  provoke: { extra: `
[이번엔 '도발·논쟁' 스타일로]
상식을 정면으로 들이받아 "뭐라고?" 하게. 비싼 거 까기·"남들 다 틀렸다"·"하지 마라" 식.
- 결: "광택? 그거 돈 버리는 겁니다" / "그 비싼 거 아직도 돈 주고 하세요?" / "남들 다 틀리게 알고 있는 그것".` },
  emotion: { extra: `
[이번엔 '따뜻한 감성 스토리' 스타일로]
자극 대신 여운. 울컥하거나 마음이 가는 진짜 사람 이야기 한 장면으로.
- 결: "돌아가신 아버지 차라고, 딱 한 번만요" / "폐차 전에 새 차처럼 보내주고 싶다셨다".` },
};

// ── Claude 호출 (raw fetch — 다른 lib 와 동일 패턴) ──
// temperature 1.0 에선 가끔 JSON 이 아닌 응답이 오므로, 파싱 실패 시 자동 재시도(retries)한다.
async function callClaude({ userMessage, system = SYSTEM, maxTokens = 8000, temperature = 0.9, retries = 2 }) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    // 재시도일수록 temperature 를 조금 낮춰 JSON 안정성을 높인다(다양성은 첫 시도에서 확보).
    const temp = attempt === 0 ? temperature : Math.max(0.4, temperature - 0.3 * attempt);
    ...
    if (!r.ok) {
      ...
      // 과부하(429/503/529)는 잠깐 쉬고 재시도, 그 외 클라이언트 오류는 즉시 throw.
      const transient = status === 429 || status === 503 || status === 529;
      if (transient && attempt < retries) { await new Promise((s) => setTimeout(s, 600 * (attempt + 1))); continue; }
    ...
    try {
      ...
      lastErr = e;   // 파싱 실패 → 다음 시도(낮은 temperature)로
    }
  }
  throw lastErr || (() => { const e = new Error("AI 응답을 JSON으로 읽지 못했습니다. 다시 시도해 주세요."); e.status = 502; return e; })();
}
...
아래 포맷 목록의 각 포맷(id)마다, 이 업종으로 만들 수 있는 '어그로 끄는 영상 제목'을 정확히 1개씩 만들어라.
- 낚시성 뉴스 헤드라인처럼 첫 1초에 멈칫하게. 결과·정답은 감추고 "그래서 어떻게 됐는데?"가 떠오르게 끊어라.
- 각 포맷의 강점에 딱 맞게(예: 비포애프터=극적인 전·후 대비 / 타임랩스=긴 시간 압축 / 후기형=반응).
- 포맷마다 소재(주제)는 자유롭게 다르게 — 한 소재에 매이지 마라.
- 설명문·슬로건 말고 '제목'으로. 25자 이내, 한국어 구어체, 이모지·해시태그·따옴표 없이.
...
출력 JSON(정확히 이 형식):
{
  "items": [ {"f":"a1","i":"3년 찌든때 차, 세차 한 번에 중고값 200 뛴 이유"} ],
...
async function generateHooks(body) {
  ...
  const style = String(body?.style || "scene").trim();
  const styleExtra = (HOOK_STYLES[style] || HOOK_STYLES.scene).extra;
  const avoidArr = Array.isArray(body?.avoid)
...
이 포맷("${format || "자유"}")에 딱 맞는 '어그로 끄는 영상 제목'을 정확히 10개 만들어라.
- ★ 가장 중요: 10개 모두 소재(주제)를 완전히 다르게. 한 주제에 매이지 마라. (특정 소재에 대한 변형 10개 금지)
- 낚시성 뉴스 헤드라인처럼 첫 1초에 멈칫. 결과·정답은 감추고 "그래서 어떻게 됐는데?"가 떠오르게 끊어라.
- 이 포맷의 강점을 살려라(예: 비포애프터=극적인 전·후 대비 / 타임랩스=긴 시간 압축 / 후기형=반응).
- 설명문·슬로건 말고 '제목'으로. 25자 이내, 한국어 구어체, 이모지·해시태그·따옴표 없이.
- 10개 톤·문장끝 모두 다르게. "~될까 / 믿어? / 끝까지 봐 / 직접 보세요"류 절대 금지.
${styleExtra}${avoidBlock}
출력 JSON: { "hooks": ["제목1","제목2","제목3","제목4","제목5","제목6","제목7","제목8","제목9","제목10"] }
JSON 외 아무 것도 출력하지 마라.`;
  ...
  return { ok: true, industry, idea, style, hooks: hooks.slice(0, 10) };
}
