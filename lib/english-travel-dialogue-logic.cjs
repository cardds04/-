...
function utteranceSimilarity(a, b) {
  const first = normalizedEnglish(a);
  const second = normalizedEnglish(b);
  if (!first || !second) return 0;
  if (first === second) return 1;
  if (first.includes(second) || second.includes(first)) return 0.9;
  return Math.max(0, 1 - levenshteinDistance(first, second) / Math.max(first.length, second.length));
}

/**
 * How many trailing history turns contain essentially this same learner
 * sentence. A growing streak means the recognizer is failing the child, not
 * the other way around, so understanding rules relax step by step.
 */
function repeatedReplyStreak(reply, history = []) {
  let streak = 0;
  for (let index = history.length - 1; index >= 0; index -= 1) {
    const learner = history[index] && history[index].learner;
    if (!learner || utteranceSimilarity(reply, learner) < 0.72) break;
    streak += 1;
  }
  return streak;
}
...
function buildMissionPrompt({ mission, location, reply, alternatives, speechConfidence, history, currentNpcLine, completedGoalIds, attemptCount, audioUnderstanding, repeatStreak = 0 }) {
  ...
  return [
    ...
    repeatStreak >= 1
      ? `Repeat notice: the learner has now said essentially this same sentence ${repeatStreak + 1} times in a row. Speech recognition is likely failing them, not their English. Interpret the utterance as generously as a kind human listener would, choose the most plausible mission-relevant meaning, mark the matching goals, and do not ask for another repetition.`
      : "",
    confidenceLine,
...
function normalizeMissionAssessment(raw, mission, input) {
  ...
  const claimedNewGoals = Array.isArray(raw && raw.newlyCompletedGoals)
    ...
    : [];
  const repeatStreak = Math.max(0, Math.floor(Number(input.repeatStreak) || 0));
  // Third try of essentially the same utterance: the recognizer, not the
  // child, is failing. Trust the model's semantic claims without the strict
  // evidence/validator gate.
  const rawNewGoals = repeatStreak >= 2
    ? claimedNewGoals
    : claimedNewGoals.filter((item) => validatesGoalEvidence(item.id, input.reply, item.evidence, safeAlternatives, correctionCandidate));
  ...
  let safeNewGoals = rawSafety !== "safe" || !understood || !relevant
    ? []
    : [...new Set(rawNewGoals.map((item) => item.id).filter((id) => !previousGoals.includes(id)))];
  // Ultimate escape hatch: three identical attempts and the machine still
  // cannot make sense of the audio. A human clerk would give the traveler the
  // benefit of the doubt — let them through to the next goal.
  let forcedPass = false;
  if (repeatStreak >= 2 && rawSafety === "safe" && !safeNewGoals.length && !understood) {
    const firstMissing = mission.goals.map((goal) => goal.id).find((id) => !previousGoals.includes(id));
    if (firstMissing) {
      safeNewGoals = [firstMissing];
      forcedPass = true;
    }
  }
  ...
  const missionComplete = missingGoalIds.length === 0 && ((understood && relevant) || forcedPass) && rawSafety === "safe";

  let decision = "continue";
  if (rawSafety !== "safe") decision = "safety";
  else if (!understood && !forcedPass) decision = "clarify";
  else if (!relevant && !forcedPass) decision = "redirect";
  ...
  const verificationReply = rejectedGoalClaims && !missionComplete && !fixedReplies[decision]
    ...
    : null;
  const forcedReply = forcedPass
    ? {
      en: missionComplete ? "Alright, all set. Let's move on!" : "Alright, I've got it. Let's keep going!",
      ko: missionComplete ? "좋아요, 다 됐어요. 계속 진행할게요!" : "네, 알겠어요. 계속 진행할게요!",
    }
    : null;
  const fixed = fixedReplies[decision] || verificationReply || forcedReply;
  ...
  return {
    ...
    conversationValid: ((understood && relevant) || forcedPass) && rawSafety === "safe",
    missionComplete,
    ...
    corrected,
    explanationKo: fixed
      ? (fixed === verificationReply
        ? verificationExplanation
        : fixed === forcedReply
          ? "기계가 목소리를 계속 못 알아들었네요. 같은 말을 여러 번 잘 말해 줬으니 그대로 통과할게요!"
          : feedbackDefaults[decision])
...
async function handleMissionDialogue(input, options = {}) {
  ...
  const repeatStreak = repeatedReplyStreak(reply, history);
  ...
  // The learner already repeated this sentence after a clarification. Asking
  // to confirm again is the loop the child cannot escape — evaluate instead.
  if (audioUnderstanding?.needsConfirmation && repeatStreak === 0) {
  ...
  const prompt = buildMissionPrompt({ mission, location, reply, alternatives, speechConfidence, history, currentNpcLine, completedGoalIds, attemptCount, audioUnderstanding, repeatStreak });
  ...
  try {
    ...
    const normalized = normalizeMissionAssessment(parsed, mission, { reply, alternatives, speechConfidence, completedGoalIds, attemptCount, currentNpcLine, repeatStreak });
