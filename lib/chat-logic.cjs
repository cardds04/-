...
- 서울·경기 외(평택·오산·천안·아산) 출장비 10~20만원 별도 — 정확한 금액은 담당자 확정
...
### 숏폼(스토리형) 제작 의뢰 방법
- **인로그로 촬영했던 현장**: **스케줄등록 어플(사이트) 하단의 [숏폼신청] 탭**에서 신청. 촬영한 곳을 고르고 ①'알아서 해주세요' 또는 ②'원하는 내용 넣기' 중 선택하면 끝. (‼️사이트 주소·URL을 직접 알려주지 말고, 항상 '스케줄등록 어플(사이트) 하단의 숏폼신청 탭'으로만 안내할 것)
- **인로그로 촬영하지 않은 곳**: 같은 [숏폼신청] 탭에서 '인로그 촬영 현장이 아니에요'를 선택해 신청하되,
