...
/**
 * 작가 토큰 → 그 작가의 { loginId, password } 를 service_role 로 조회해 반환.
 * shoot-* 엔드포인트가 평문 비밀번호 대신 토큰만 받고도 동작하도록 하는 다리.
 * (브라우저가 비밀번호를 더 이상 보낼 필요가 없어진다 — 비밀번호 노출 제거 대비.)
 * 토큰 무효/작가 없음이면 null.
 */
async function resolveWriterCredsFromToken(token) {
  const claims = verifyWriterToken(token);
  if (!claims) return null;
  const loginId = norm(claims.lid);
  if (!loginId) return null;
  const rows = await fetchRowsByLoginId(loginId);
  const w = rows[0];
  if (!w) return null;
  return { loginId: norm(w.login_id) || loginId, password: String(w.password != null ? w.password : "") };
}
...
module.exports = {
  handleWriterAuthRequest,
  verifyWriterToken,
  resolveWriterCredsFromToken,
  WRITER_TTL_SECONDS,
};
