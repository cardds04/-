...
// 업체당 최대 3개 번호(쉼표 등 구분) → 유효 휴대폰 배열(중복 제거). 단일 번호도 [single] 로 동작.
...
/** 편집완료 파일 보관 기간(개월). 이 기간 안에 다운로드하도록 문자로 안내한다. */
const DELIVERY_RETENTION_MONTHS = 1;
 ...
 *
 * 사진·영상이 모두 완료된 시점에만 1통 보내며 "편집완료" 로 통일.
 * (사진만/영상만 업체도 동일 표현.)
 * 형식 예: 「쇼픽알림」 / 「0429현장 편집완료(1개월 보관)」 / URL 단독 줄
 *
 * ‼️90바이트(단문 SMS 한계)를 넘기지 말 것 — 넘으면 장문(LMS)으로 자동 전환돼 건당 요금이 약 3배.
 *   현재 최악(브랜드 "인로그"/"더필링" + URL 25바이트) 기준 81바이트, 여유 9바이트.
 *   한글 1자 = 3바이트라 문구를 3자 이상 늘리면 초과한다. 옛 문구의 "촬영본"을 뺀 이유가 이것.
 *   (실측 2026-07-22: 등록된 폴더 URL 258개가 전부 25바이트로 균일)
 */
function buildDeliveryCompletionSmsText(opts) {
  ...
  // 보관기간 안내를 둘째 줄 괄호로 합침 — 단문(90바이트) 유지하면서 기한 내 다운로드를 알림.
  const lines = [`${brand}알림`, `${siteLine} 편집완료(${DELIVERY_RETENTION_MONTHS}개월 보관)`];
...
async function purgeOriginalFoldersOlderThanRetention(writeHeaders, log) {
  ...
  // ‼️무제한 GET 은 1000행 캡 잘림 — 초과분 원본폴더가 보존 정리에서 누락됨(2026-07-29)
  const stateRows = await loadAllDeliveryStateRows(writeHeaders);
...
/**
 * ‼️shoot_delivery_drive_state 전량 로드는 반드시 페이지네이션(2026-07-29 실사고 계열).
 * 무제한 `?select=*` 는 PostgREST max-rows(1000)에서 조용히 잘린다 — 당시 전체 1,027행.
 * 잘린 스케줄은 stateById 에 없어 row=null 로 취급되고, 폴더 생성 경로의 upsert 가
 * 납품완료(photo/video_notified_at)를 null 로 되돌릴 수 있었다(payload 쪽 방어도 별도 적용:
 * photographer-shoot-logic.cjs ensureFolderTreeForScheduleNaver 의 null 키 제거).
 */
async function loadAllDeliveryStateRows(headers) {
  const pageSize = 1000;
  const out = [];
  // 종료 조건은 '빈 페이지'만 — `rows.length < pageSize` 로 끊으면 서버 max-rows 가
  // pageSize 보다 낮게 설정된 환경에서 조용한 잘림이 재발한다(사고 원인과 같은 침묵 실패).
  // offset 은 실제 회수량(out.length) 기준으로 전진 — 서버가 몇 행을 주든 전량을 돈다.
  for (let guard = 0; guard < 100; guard += 1) {
    const rows = await supabaseJson(
      `shoot_delivery_drive_state?select=*&order=schedule_id.asc&limit=${pageSize}&offset=${out.length}`,
      headers
    );
...
async function runDeliveryDriveJob(log = console.log.bind(console)) {
  ...
  const [schedules, directory, stateRows] = await Promise.all([
    ...
    loadAllDeliveryStateRows(writeH),
  ]);
...
async function previewDeliveryDriveFolders() {
  ...
  const [schedules, stateRows, directoryRows] = await Promise.all([
    ...
    loadAllDeliveryStateRows(writeH), // ‼️무제한 GET 은 1000행 캡 잘림 — preview 도 잡과 같은 기준으로 판단해야 함(2026-07-29)
    supabaseJson(
 ...
 */
async function sendDashboardDeliverySms({ scheduleId, kind, customerPhoneOverride, skipSms }) {
  ...
  // skipSms(무료촬영 건 등) — 완료 표시만 하고 어떤 경우에도 문자는 보내지 않는다.
  const shouldSendSms = skipSms === true ? false : bothNeeded ? otherAlreadyDone : true;
  ...
  // 수신자 목록: 수동 입력(override)이면 그 번호 하나, 아니면 디렉터리의 모든 번호(업체당 최대 3).
  const dirRowForPhones = pickCompanyDirectoryRow(directory, companyName, companyCode);
  const directoryRawPhone = dirRowForPhones ? String(dirRowForPhones.customer_phone || "") : "";
  let recipients;
  if (wantedOverride) {
    recipients = isValidKoreanMobile(fromOverride) ? [fromOverride] : [];
  } else {
    recipients = splitCustomerPhones(directoryRawPhone);
    if (!recipients.length) {
      // 디렉터리에 없으면 예전 캐시(state.customer_phone)·단일 lookup 으로 폴백.
      const single =
        normalizePhoneFromDirectory(row.customer_phone) ||
        normalizePhoneFromDirectory(pickCustomerPhoneFromDirectory(directory, companyName, companyCode));
      if (isValidKoreanMobile(single)) recipients = [single];
    }
  }
  ...
  if (shouldSendSms) {
    if (wantedOverride && !isValidKoreanMobile(fromOverride)) {
      return {
        ok: false,
        status: 400,
        code: "bad_phone",
  ...
  // company_name/shoot_date_key/composition 도 같이 백필해서 납품 완료 로그 행에 업체명·날짜가
  // 「-」 로 나오던 문제 방지.
  if (!shouldSendSms) {
    await patchDeliveryState(writeH, canonSid, {
      company_share_link: link,
      company_name: companyName || row.company_name || "",
      company_code: companyCode || row.company_code || "",
      shoot_date_key: shootDateYmd || row.shoot_date_key || "",
   ...
   *  업체당 최대 3개 번호 전부에 발송 — 하나라도 성공하면 성공으로 본다. */
  let anySent = false;
  let anyDeferred = false;
  let lastScheduledIso = null;
  let lastErr = "";
  for (const to of recipients) {
    const sms = await sendSolapiMessage({ to, text, referenceDate: sendRef });
    if (sms.ok) {
      anySent = true;
      if (sms.deferredToQuietHoursMorning) anyDeferred = true;
      if (sms.scheduledDate) lastScheduledIso = sms.scheduledDate;
    } else {
      lastErr = typeof sms.message === "string" ? sms.message : "문자 발송에 실패했습니다.";
    }
  }
  if (!anySent) {
  ...
  return {
    ...
    data: {
      ...
      video_notified_at: k === "video" ? notifiedAt : row.video_notified_at || null,
      sms_sent: true,
      sms_recipients: recipients.length,
