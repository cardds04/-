...
/**
 * 사진·영상 납품 완료 안내 문자 (업체 Drive 루트 폴더 링크 포함)
...
function buildDeliveryCompletionSmsText(opts) {
  ...
  const companyFolderUrl = String(opts?.companyFolderUrl || "").trim();
  ...
  const line3 = "아래 링크에서 확인해 주세요";

  const lines = [line1, line2, line3];
  if (companyFolderUrl && /^https?:\/\//i.test(companyFolderUrl)) {
    lines.push(companyFolderUrl);
  }
  return lines.join("\n");
}
