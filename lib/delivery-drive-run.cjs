...
/** company_directory.site_type → 인로그 / 쇼픽 / 더필링 (단문 문자 앞머리) */
function brandLabelFromDirectoryRow(dirRow) {
  const st = String(dirRow?.site_type ?? dirRow?.siteType ?? "").trim().toLowerCase();
  if (st === "shopick") return "쇼픽";
  if (st === "thefeeling") return "더필링";
  return "인로그";
}

/** SMS에 넣을 고객 포털 절대 URL 기준 호스트 */
function getSmsPublicOrigin() {
  const raw = String(process.env.SITE_PUBLIC_ORIGIN || process.env.PUBLIC_SITE_ORIGIN || process.env.NEXT_PUBLIC_SITE_ORIGIN || "")
    .trim()
    .replace(/\/+$/, "");
  if (raw) {
...
/** 사이트 종류별 고객 대표 페이지 경로 (/customer · /public-order lite 등) */
function customerPortalSmsUrl(siteTypeSlug) {
  const base = getSmsPublicOrigin();
  if (!base) return "";
  const st = String(siteTypeSlug || "").trim().toLowerCase();
  if (st === "shopick") return `${base}/public-order.html?mode=lite`;
  if (st === "thefeeling") return `${base}/public-order.html`;
  return `${base}/customer.html`;
}

function buildShortDeliverySmsText(directoryRows, companyName, companyCode, kind) {
  const k = kind === "video" ? "video" : "photo";
  const dirRow = pickCompanyDirectoryRow(directoryRows, companyName, companyCode);
  const brand = brandLabelFromDirectoryRow(dirRow || {});
  const portalUrl = customerPortalSmsUrl(dirRow?.site_type || dirRow?.siteType || "");
  const phrase = k === "video" ? "영상편집완료" : "사진편집완료";
  const text =
    portalUrl && /^https:\/\//i.test(portalUrl) ? `${brand} ${phrase}\n${portalUrl}` : `${brand} ${phrase}`;
  return text;
}
...
async function runDeliveryDriveJob(log = console.log.bind(console)) {
  ...
  const [schedules, directory, stateRows] = await Promise.all([
    ...
    supabaseJson("company_directory?select=name,code,customer_phone,site_type", readH),
      ...
      if (wantPhoto && row.photo_folder_id && !row.photo_notified_at) {
        ...
        if (ids.length > 0) {
          ...
          if (!skipSms && !isValidKoreanMobile(phone)) {
            ...
            const text = buildShortDeliverySmsText(directory, companyName, companyCode, "photo");
      ...
      if (wantVideo && row?.video_folder_id && !row?.video_notified_at) {
        ...
        if (ids.length > 0) {
          ...
          if (!skipSms && !isValidKoreanMobile(phoneFinal)) {
            ...
            const text = buildShortDeliverySmsText(directory, companyName, companyCode, "video");
            const sms = await sendSolapiMessage({
              to: phoneFinal,
              text,
              type: "SMS",
...
async function sendDashboardDeliverySms({ scheduleId, kind, customerPhoneOverride }) {
  ...
  const text = buildShortDeliverySmsText(directory, companyName, companyCode, k);
  const sms = await sendSolapiMessage({
    to: phone,
    text,
    type: "SMS",
