 ...
 */
const { sendSolapiMessage, peekSolapiQuietHoursPlan } = require("./solapi-logic.cjs");
const {
...
async function sendDashboardDeliverySms({ scheduleId, kind, customerPhoneOverride }) {
  ...
  const link = resolveDeliverySmsNaverWorksFolderUrl(dirRowSms, String(row.company_share_link || ""));
  ...
  if (!link) {
    return {
      ...
      status: 400,
      message:
        looksLikeGoogleDriveShareUrl(row.company_share_link)
          ? "납품 DB에 구글 드라이브 링크만 있고, 업체 디렉터리에 네이버웍스 공유 주소가 없습니다. 「업체정보관리」에서 해당 업체 「폴더 연결」로 네이버웍스 주소를 등록한 뒤 다시 발송해 주세요."
          : "업체 네이버웍스 납품 폴더 링크를 찾을 수 없습니다. 업체 디렉터리에 네이버웍스 공유 주소를 등록(또는 현장 확인으로 연동)한 뒤 다시 시도하세요.",
    };
  }
  ...
  const quietPlan = peekSolapiQuietHoursPlan(new Date());
  const notifiedAt = quietPlan.recordAtIso;
  let claimed = false;
  try {
    claimed = await patchDeliveryNotifyClaim(writeH, sid, k, notifiedAt);
  } catch (e) {
    return {
      ok: false,
      status: 500,
      message: typeof e?.message === "string" ? e.message : "발송 준비에 실패했습니다.",
    };
  }
  if (!claimed) {
    return {
      ...
      message:
        k === "video"
          ? "이미 영상 납품 안내 문자가 발송된 건입니다."
          : "이미 사진 납품 안내 문자가 발송된 건입니다.",
    };
  }
  ...
  const sms = await sendSolapiMessage({
    to: phone,
    text,
    skipQuietHoursSchedule: true,
    ...(quietPlan.scheduledDate ? { scheduledDate: quietPlan.scheduledDate } : {}),
  });
  if (!sms.ok) {
    await revertDeliveryNotifyClaim(writeH, sid, k, notifiedAt);
    return {
      ok: false,
      status: 502,
  ...
  await patchDeliveryState(writeH, sid, {
  ...
  return {
    ...
    data: {
      ...
      sms_deferred_quiet_hours: !!quietPlan.deferredToQuietHoursMorning,
      sms_scheduled_iso: quietPlan.scheduledDate || null,
    },
  };
}
