...
function buildDeliveryCompletionSmsText(opts) {
  ...
  const folderUrlFromDirectory = String(dirRow?.naver_works_company_share_link || "").trim();
  const folderUrl = companyFromCaller || folderUrlFromDirectory;
...
async function sendDashboardDeliverySms({ scheduleId, kind, customerPhoneOverride }) {
  ...
  const link =
    ...
    String(dirRowSms?.naver_works_company_share_link || "").trim();

  if (!link) {
    return {
      ok: false,
      status: 400,
      message: "업체 폴더 링크를 찾을 수 없습니다. 네이버웍스 공유 링크(또는 납품 상태에 기록된 company_share_link)를 준비한 뒤 다시 시도하세요.",
    };
  }
