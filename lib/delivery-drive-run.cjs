...
async function sendDashboardDeliverySms({ scheduleId, kind, customerPhoneOverride }) {
  ...
  let link = resolveDeliverySmsNaverWorksFolderUrl(dirRowSms, row ? String(row.company_share_link || "") : "");
  ...
  if (!link) {
    return {
      ...
      status: 400,
      message:
        row && looksLikeGoogleDriveShareUrl(row.company_share_link)
          ? "납품 DB에 구글 드라이브 링크만 있고, 업체 디렉터리에 네이버웍스 공유 주소가 없습니다. 「업체정보관리」에서 해당 업체 「폴더 연결」로 네이버웍스 주소를 등록한 뒤 다시 발송해 주세요."
          : "업체 네이버웍스 납품 폴더 링크를 찾을 수 없습니다. 「업체정보관리」에서 해당 업체 「폴더 연결」로 네이버웍스 공유 주소를 등록한 뒤 다시 시도하세요.",
    };
  }
