...
async function handleSign(body) {
  ...
  const isInput = !!body.input;   // 🎬 고객이 올린 '원본 입력 영상' → in/ 폴더(완성본 cust/ 와 분리) + 확장자 더 허용
  let ext = norm(body.ext).toLowerCase().replace(/[^a-z0-9]/g, "");
  if (ext === "quicktime") ext = "mov";
  const OK = ["mp4", "webm", "mov", "m4v"];
  if (!OK.includes(ext)) ext = isInput ? "mp4" : "webm";
  const ts = Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
  const path = `${isInput ? "in" : "cust"}/${safeSeg(u.lid)}/${ts}.${ext}`;   // 모두 URL-safe → 인코딩 불필요
