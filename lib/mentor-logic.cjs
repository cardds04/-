...
async function loadAllMessages() {
  const rows = await sb(
    `mentor_messages?owner_id=eq.${OWNER_ID}&order=created_at.asc`
  );
...
async function loadAllAssignments() {
  const rows = await sb(
    `mentor_assignments?owner_id=eq.${OWNER_ID}&order=created_at.asc`
  );
  return Array.isArray(rows) ? rows : [];
}
...
// ------------------------------------------------------------
// 대화 백업 내보내기 — Claude Code 에 그대로 건네줄 수 있는 Markdown
// ------------------------------------------------------------
function buildExportMarkdown(profile, assignments, messages) {
  const p = profile || {};
  const now = new Date().toISOString();
  const lines = [];
  lines.push("# AI 경영 멘토 — 대화 백업");
  lines.push("");
  lines.push(`> 내보낸 시각: ${now}`);
  lines.push(
    "> 이 파일은 Claude Code 에게 건네서 멘토 대화의 맥락을 이어가기 위한 백업입니다."
  );
  lines.push("");

  lines.push("## 사업 프로필 (멘토의 기억)");
  const pf = [
    ["사업", p.business],
    ["팀", p.team],
    ["고객", p.customers],
  ...
  for (const [label, val] of pf) {
    if (val && String(val).trim()) {
      lines.push(`- **${label}:** ${String(val).trim()}`);
      hasProfile = true;
    }
  }
  if (!hasProfile) lines.push("- (아직 기록된 프로필이 없습니다)");
  lines.push("");
  ...
  if (!list.length) {
    ...
    for (const a of list) {
      const mark = a.status === "done" ? "[x]" : a.status === "dropped" ? "[-]" : "[ ]";
      const due = a.due_date ? ` (기한: ${a.due_date})` : "";
      lines.push(`- ${mark} **${a.title || "(제목 없음)"}**${due}`);
      if (a.detail && String(a.detail).trim()) {
        lines.push(`  - ${String(a.detail).trim()}`);
      }
    }
  }
  lines.push("");
  ...
  if (!msgs.length) {
    lines.push("_(대화 없음)_");
  }
  for (const m of msgs) {
    const when = m.created_at ? String(m.created_at).replace("T", " ").slice(0, 16) : "";
    if (m.role === "user") {
      lines.push(`### 🙋 대표  ${when ? `· ${when}` : ""}`.trim());
      lines.push("");
      lines.push(String(m.content || "").trim());
    } else {
      lines.push(`### 🏛️ 멘토  ${when ? `· ${when}` : ""}`.trim());
      lines.push("");
      lines.push(String(m.content || "").trim());
      const panel = m.panel && typeof m.panel === "object" ? m.panel : {};
      const council = Array.isArray(panel.council) ? panel.council : [];
      if (council.length) {
        lines.push("");
        lines.push("<details><summary>자문단 회의 (근거)</summary>");
        lines.push("");
        for (const c of council) {
          lines.push(`- **${c.role || "자문위원"}:** ${c.take || ""}`);
        }
        lines.push("");
        lines.push("</details>");
      }
      if (panel.stage) lines.push(`\n_단계: ${panel.stage}_`);
    }
    lines.push("");
    lines.push("---");
    lines.push("");
...
async function handleExport() {
  const [profile, assignments, messages] = await Promise.all([
    loadProfile(),
    loadAllAssignments(),
    loadAllMessages(),
  ]);
  const markdown = buildExportMarkdown(profile, assignments, messages);
  const stamp = new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-");
  return {
    status: 200,
    json: {
      ok: true,
      filename: `멘토-대화백업-${stamp}.md`,
      markdown,
      counts: {
