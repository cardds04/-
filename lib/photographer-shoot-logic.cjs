...
async function completePhotographerShoot({
  ...
  let rowForResponse = await fetchDeliveryState(headers, schedule.id);
  if (!rowForResponse) {
    rowForResponse = { ...row, ...patchRow };
  } else {
    rowForResponse = {
      ...row,
      ...rowForResponse,
  ...
  for (const sid of ids) {
    ...
    if (rowNeedsSubfolderHeal(st, schedule.composition)) {
      try {
        ...
        console.warn("[listPhotographerShootPanel] 하위폴더 치료 실패", sid, err?.message || err);
      }
    }
