 ...
 * 트리를 만든다. 이미 있으면 재사용만 하고 새로 만들지 않는다(reuseIfExists).
...
async function ensureWriterDeliverySubfolders({ writerLoginId, writerPassword, scheduleId, folderName }) {
  ...
  let companyFolderId = String(dirRow?.naver_works_company_folder_id || "").trim();
  let companyShareLink = String(dirRow?.naver_works_company_share_link || "").trim();
  let provisionedCompanyFolder = false;
  if (!companyFolderId) {
    // 완전 신규 업체 — 공유폴더 안에 업체 폴더를 만들고 공유링크를 발급해
    // company_directory(폴더연결 주소)에 저장까지 한다(2026-08-07 사장님 지시).
    try {
      const out = await provisionNaverWorksCompanyDirectoryFolder({
        supabaseHeaders: { url: headers.url, headers: headers.headers },
        directoryRow: dirRow,
      });
      companyFolderId = String(out.folderId || "").trim();
      companyShareLink = String(out.shareLink || "").trim() || companyShareLink;
      provisionedCompanyFolder = Boolean(out.createdFolder);
    } catch (e) {
      const msg = typeof e?.message === "string" ? e.message : "업체 폴더 생성 실패";
      return { ok: false, status: 502, message: `업체 폴더를 만들지 못했습니다: ${msg}` };
    }
  }
  if (!companyFolderId) {
    return {
      ok: false,
      status: 409,
      message: "이 업체의 마이박스 납품 폴더가 아직 없습니다. 관리자에게 폴더 생성을 요청해 주세요."
    };
  }

  const ro = { reuseIfExists: true };
  const shootOut = runNaverWorksDriveFolderScript(shootFolderName, companyFolderId, ro);
  ...
  const made = { shootFolder: shootFolderName, photo: "", video: "" };
  if (wantPhoto || !wantVideo) {
    const nm = `${mmdd}${compForName}사진원본`;
    const x = runNaverWorksDriveFolderScript(nm, shootOut.fileId, ro);
  ...
  made.provisionedCompanyFolder = provisionedCompanyFolder;
