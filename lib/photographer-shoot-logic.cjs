...
const { provisionCompanyDirectoryFolder, provisionNaverWorksCompanyDirectoryFolder } = require("./company-drive-provision.cjs");
...
function runNaverWorksDriveFolderScript(folderName, parentFileId) {
  ...
  if (createPy) {
    ...
    const r = spawnSync(bin, [createPy, "--folder-name", fname, "--parent-file-id", pid], {
      encoding: "utf8",
    ...
    if (r.error) {
      ...
      if (code === "ENOENT") {
        return {
          ok: false,
          message:
            "Python 실행 파일을 찾을 수 없습니다. NAVER_WORKS_PYTHON_BIN(예: /opt/venv/bin/python 또는 python3)을 설정하고, 로컬이면 scripts/에 PyJWT·requests를 설치해 주세요.",
        };
      }
  ...
  const r = spawnSync(bin, [script, "--folder-name", fname, "--parent-file-id", pid], {
    encoding: "utf8",
    maxBuffer: 10 * 1024 * 1024,
    env: process.env,
    cwd: repoRoot,
...
async function createNaverWorksFolderForPhotographer({ writerLoginId, writerPassword, scheduleId }) {
  ...
  const out = runNaverWorksDriveFolderScript(folderName, parentFileId);
  if (!out.ok) {
...
function buildMinimalShootDeliveryState(schedule, directoryRows) {
  ...
  const place = String(schedule.place || "").trim();
  const dirRow = findCompanyDirectoryRow(directoryRows, companyName, companyCode);
  const companyFolderId = String(dirRow?.google_drive_company_folder_id || "").trim() || null;
  const companyShare =
    String(dirRow?.google_drive_company_share_link || "").trim() ||
    (companyFolderId ? webViewLinkFromId(companyFolderId) : "") ||
    "";
...
async function completePhotographerShoot({
  ...
  const shareFromDirectory =
    String(dirRowForShare?.google_drive_company_share_link || "").trim() ||
    (String(dirRowForShare?.google_drive_company_folder_id || "").trim()
      ? webViewLinkFromId(String(dirRowForShare.google_drive_company_folder_id).trim())
      : "");
  ...
  if (!skipImage) {
    ...
    if (!signaturePublicUrl) {
      const drive = getDriveClient();
      const parentFolderId = getParentFolderId();
      if (parentFolderId && row?.shoot_folder_id) {
        try {
          row = await mergeShootCompositionSubfoldersIntoRow(drive, headers, schedule, row);
        } catch (mergeErr) {
          console.warn("[completePhotographerShoot] 하위폴더 동기 생략", mergeErr?.message || mergeErr);
        }
      }
      const uploadParentId = siteConfirmDriveParentFolderId(row, schedule.composition);
      if (uploadParentId && parentFolderId) {
        ...
        console.warn(
          "[completePhotographerShoot] Drive 업로드 생략 — 납품 폴더가 없거나 Storage만 사용. 「폴더만들기」로 폴더를 준비할 수 있습니다."
        );
      }
    }
  }
  ...
  return {
    ...
    data: {
      ...
      company_folder_web_link:
        ...
        (String(rowForResponse?.company_folder_id || "").trim()
          ? webViewLinkFromId(String(rowForResponse.company_folder_id).trim())
          : "") ||
        shareFromDirectory ||
        "",
...
async function provisionPhotographerCompanyDeliveryFolder({ writerLoginId, writerPassword, scheduleId }) {
  ...
  const googleDriveIdEarly = String(dirRow.google_drive_company_folder_id || "").trim();
  if (!naverFidEarly && googleDriveIdEarly) {
    const link =
      String(dirRow.google_drive_company_share_link || "").trim() ||
      webViewLinkFromId(googleDriveIdEarly);
    return {
      ok: true,
      data: {
        shareLink: link,
...
/**
 * DB shoot_folder_id 가 이 스케줄 기준으로 Drive 에 실제 존재하는지(이름·업체 폴더 하위) 검사.
 * 날짜·장소 변경·수동 삭제·오래된 id 만 남은 경우 false → 폴더 재생성 플로우로 진행.
 */
async function verifyStoredShootFolderMatchesSchedule(drive, st, schedule, directoryRows) {
  ...
  const shootDate = String(schedule.date_key || "").trim();
  const place = String(schedule.place || "").trim();
  const expectedName = buildShootFolderName(shootDate, place);
  ...
  if (!meta || meta.trashed === true) return false;
  if (String(meta.mimeType || "") !== "application/vnd.google-apps.folder") return false;
  if (String(meta.name || "").trim() !== String(expectedName).trim()) return false;
  const parents = Array.isArray(meta.parents) ? meta.parents : [];
  if (!parents.includes(companyFolderId)) return false;
  return true;
}

/**
 * 관리자 메인보드: 해당 스케줄 촬영일 현장(Drive) 폴더가 없으면 생성·DB 반영.
 * DB 에 shoot_folder_id 가 있어도 Drive·일정·장소와 맞을 때만 "이미 있음" 처리.
 */
async function ensureShootDriveFoldersAsAdmin({ scheduleId }) {
  ...
  if (st && String(st.shoot_folder_id || "").trim()) {
    const matches = await verifyStoredShootFolderMatchesSchedule(drive, st, schedule, directoryRows);
    if (matches) {
      return {
        ok: true,
        data: {
