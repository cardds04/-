/**
 * 작가 페이지: 현장 사진 업로드 + Drive 업체 폴더 트리 생성 + 완료 표시
 */
const {
...
async function verifyWriter(headers, loginId, password) {
  ...
  const rows = await supabaseJson(
    `writers?login_id=eq.${encodeURIComponent(id)}&select=name,login_id,password&limit=1`,
    headers
  );
  const w = Array.isArray(rows) ? rows[0] : null;
  if (!w || String(w.password) !== pw) return null;
  return { name: String(w.name || "").trim(), loginId: String(w.login_id || "").trim() };
}
...
async function completePhotographerShoot({
  ...
  if (!buf || !Buffer.isBuffer(buf) || buf.length === 0) {
    return { ok: false, status: 400, message: "현장 사진 파일이 필요합니다." };
  }
  ...
  if (existing?.photographer_site_done_at) {
    return { ok: false, status: 409, message: "이미 촬영 완료 처리된 건입니다." };
  }
  ...
  const siteFolderId = await ensureFolder(drive, row.shoot_folder_id, "현장");
  const safeBase = `현장사진_${Date.now()}${extFromMime(mimeType || "image/jpeg")}`;
  const fileId = await uploadBufferToFolder(drive, {
  ...
  await patchDeliveryState(headers, String(schedule.id), {
    ...
    photographer_site_file_id: fileId,
...
async function completeShootSiteAsAdmin({ scheduleId, fileBuffer, mimeType }) {
  ...
  let existing = await fetchDeliveryState(headers, schedule.id);
  if (existing?.photographer_site_done_at) {
    return { ok: false, status: 409, message: "이미 현장 촬영 완료 처리된 건입니다." };
  }
  ...
  if (fileId) patch.photographer_site_file_id = fileId;
