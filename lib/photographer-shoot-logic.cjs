...
const { companyFolderName } = require("./company-folder-name.cjs");
...
/**
 * 옛 하위폴더(사진편집완료·사진원본파일·영상편집완료·영상원본파일) — ‼️2026-09-23 폐지, 아무것도 만들지 않는다.
 * 08-11 릴레이 재배선 뒤로 이 경로가 마이박스에 실제로 빈 폴더 4개를 만들고 있었다(작가 「원본 업로드 완료」·작가 화면 열기 때).
 * 납품 하위폴더는 아침 루틴(scripts/daily_delivery_folders.cjs)이 만드는 `{MMDD}{업체}사진원본`·`영상원본` 두 개뿐이다.
 */
function ensureShootCompositionSubfoldersNaverSync(_shootFolderId, _composition) {
  return {
...
function ensureFolderTreeForScheduleNaver(schedule, directoryRows, existing) {
  ...
  // ‼️진행상태 컬럼 스테일 덮어쓰기 방지(2026-07-29): 이 함수는 폴더 프로비저닝 담당일 뿐,
  //   납품완료(photo/video_notified_at)·작가확인 타임스탬프·파일감지 이력의 주인이 아니다.
  //   existing 스냅샷이 낡았을 때 그 값을 upsert 하면 양방향 사고가 난다 —
  //   ①스냅샷 null → DB 의 실제 완료 시각을 null 로 복귀(state 전량 로드가 PostgREST
  //     1000행 캡에서 잘렸거나, 읽기→쓰기 사이에 대시보드 notify 가 끼어든 레이스),
  //   ②스냅샷 non-null → 관리자 revert 로 null 이 된 값을 되살림.
  //   merge-duplicates 는 payload 에 있는 컬럼만 갱신하므로, 이 함수가 주인이 아닌 컬럼은
  //   값과 무관하게 **키 자체를 항상 뺀다**(신규 insert 는 컬럼 기본값 — notified 계열 null,
  //   seen_file_ids '[]'::jsonb — 이라 결과 동일. 스키마 확인함).
  for (const k of [
    "photo_notified_at",
    "video_notified_at",
    "photographer_site_done_at",
    "photographer_site_file_id",
    "photographer_site_signature_url",
...
// customer_phone 에 한 업체당 여러 번호(최대 3개)가 쉼표 등으로 구분돼 저장될 수 있다.
// 각 번호를 정규화해 유효한 한국 휴대폰만 추려 배열로 반환(중복 제거, 최대 3개).
// 단일 번호("01012345678")는 [single] 로 그대로 동작 — 하위호환.
function splitCustomerPhones(raw) {
  const parts = String(raw || "").split(/[,;/\n|]+/);
  const out = [];
  for (const p of parts) {
    const norm = normalizePhoneFromDirectory(p);
    if (norm && isValidKoreanMobile(norm) && !out.includes(norm)) out.push(norm);
    if (out.length >= 3) break;
  }
  return out;
}

// 매칭된 디렉터리 행의 raw customer_phone(쉼표 결합 가능) 반환.
function pickCustomerPhoneRawFromDirectory(directoryRows, companyName, companyCode) {
  const name = String(companyName || "").trim();
  const code = String(companyCode || "").trim();
  const rows = Array.isArray(directoryRows) ? directoryRows : [];
  const nameMatches = rows.filter((r) => String(r?.name || "").trim() === name);
  if (nameMatches.length === 1) return String(nameMatches[0]?.customer_phone || "");
  if (name && code) {
    const hn = nameMatches.find((r) => String(r?.code || "").trim() === code);
    if (hn) return String(hn?.customer_phone || "");
    const hc = rows.filter((r) => String(r?.code || "").trim() === code);
    if (hc.length === 1) return String(hc[0]?.customer_phone || "");
  }
  return "";
}

// 업체의 모든 고객 번호(유효 휴대폰, 최대 3개) 배열.
function pickCustomerPhonesFromDirectory(directoryRows, companyName, companyCode) {
  return splitCustomerPhones(pickCustomerPhoneRawFromDirectory(directoryRows, companyName, companyCode));
}
...
async function sendPhotographerSiteConfirmToCustomer({ customerPhone, writerName, writerPhone }) {
  // customerPhone 에 여러 번호(최대 3개)가 들어올 수 있다 → 전부에게 발송.
  const recipients = splitCustomerPhones(customerPhone);
  ...
  for (const to of recipients) {
    ...
    if (sms.ok) {
      anySent = true;
      if (sms.deferredToQuietHoursMorning) anyDeferred = true;
      if (sms.recordAtIso) lastRecordAt = sms.recordAtIso;
    } else {
  ...
  return { result: "sent", deferred: anyDeferred, recordAtIso: lastRecordAt };
}
...
async function notifyPhotographerPreCustomerSms({ writerLoginId, writerPassword, scheduleId, kind }) {
  ...
  // 업체당 최대 3개 번호 전부에 발송.
  const phones = pickCustomerPhonesFromDirectory(
    directoryRows,
  ...
  // 모든 번호로 발송 — 하나라도 성공하면 성공으로 본다(나머지 실패는 메시지로 안내).
  let anySent = false;
  let anyDeferred = false;
  ...
  for (const to of phones) {
    ...
    if (sms.ok) {
      anySent = true;
      if (sms.deferredToQuietHoursMorning) anyDeferred = true;
      if (sms.scheduledDate) lastScheduledIso = sms.scheduledDate;
    } else {
      lastErr = typeof sms.message === "string" ? sms.message : "문자 발송 실패";
    }
  }
  if (!anySent) {
    return { ok: false, status: 502, message: lastErr || "문자 발송 실패" };
  }
  return {
    ok: true,
    kind: k,
    ...
    sms_deferred_quiet_hours: anyDeferred,
    sms_scheduled_iso: lastScheduledIso,
  };
}
 ...
 */
/**
 * 마이박스(공유폴더) 폴더 생성 — 로컬 맥 릴레이(ngrok) 경유.
 * ‼️납품 폴더의 실체는 네이버웍스 드라이브가 아니라 사장님 마이박스 「공유폴더」(2026-08-11 확인).
 *   릴레이가 크롬 마이박스 쿠키로 mkdir.api 를 호출하며, 이미 있으면 기존 폴더 키를 반환한다.
 * parent: 마이박스 공유링크(naver.me/…) 또는 resourceKey.
 */
async function relayCreateMyboxFolder(folderName, parent) {
  const relayBase = String(process.env.NAVER_WORKS_RELAY_URL || "").trim().replace(/\/+$/, "");
  if (!relayBase) {
    return { ok: false, message: "릴레이 서버 주소(NAVER_WORKS_RELAY_URL)가 설정되지 않았습니다." };
  }
  const relaySecret = String(process.env.NAVER_WORKS_RELAY_SECRET || "").trim();
  ...
  try {
    const res = await fetch(`${relayBase}/createfolder`, {
      method: "POST",
      headers,
      body: JSON.stringify({ folderName: String(folderName || "").trim(), parentFileId: String(parent || "").trim() })
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || !data.ok || !data.fileId) {
      return { ok: false, message: String(data?.message || `릴레이 응답 오류 (HTTP ${res.status})`) };
    }
    return { ok: true, fileId: String(data.fileId), reused: Boolean(data.reused) };
  } catch (e) {
    return { ok: false, message: `릴레이 서버에 연결하지 못했습니다(맥이 꺼져 있거나 터널 중단): ${e?.message || e}` };
  }
}

/**
 * 작가 「폴더생성후열기」(2026-08-07 사장님 지시) — 업체 납품 폴더 안에
 *   {만들폴더명}/                      예: 0701206호1  (작가 화면에 표시되는 이름 그대로)
 *     ├ {MMDD}{업체명}사진원본/
 *     └ {MMDD}{업체명}영상원본/        (촬영구성에 따라 필요한 것만)
 * 트리를 만든다. 이미 있으면 재사용만 하고 새로 만들지 않는다.
 * 원본 수집 자동화(mybox_fetch)는 날짜폴더 + "사진원본"/"영상원본" 부분일치로 찾으므로 호환.
 */
async function ensureWriterDeliverySubfolders({ writerLoginId, writerPassword, scheduleId, folderName }) {
  const headers = getSupabaseServiceHeaders();
  const writer = await verifyWriter(headers, writerLoginId, writerPassword);
  if (!writer) {
    return { ok: false, status: 401, message: "작가 로그인 정보가 올바르지 않습니다." };
  ...
  const companyCode = String(schedule.code || "").trim();
  const shootDate = String(schedule.date_key || "").trim();
  const mmdd = shootDate.replace(/-/g, "").slice(4, 8);
  if (!mmdd || mmdd.length !== 4) {
    return { ok: false, status: 400, message: "촬영일이 없어 폴더 이름을 만들 수 없습니다." };
  }

  // 만들폴더명: 작가 화면 계산값을 그대로 쓰되 파일명 금지문자 제거·40자 컷. 비면 서버 규칙 폴백.
  let shootFolderName = String(folderName || "")
    .replace(/[\/\\:*?"<>|]/g, "")
    .trim()
    .slice(0, 40);
  if (!shootFolderName) shootFolderName = buildShootFolderName(shootDate, schedule.place);
  if (!shootFolderName) {
    return { ok: false, status: 400, message: "폴더 이름을 만들 수 없습니다." };
  }
  ...
  if (!dirRow?.id) {
    return {
      ok: false,
      status: 409,
      message: `「${companyName}」 업체가 업체정보관리에 없습니다. 관리자에게 업체 등록을 요청해 주세요.`
    };
  }
  // 마이박스 접근은 「폴더연결 주소」(공유링크)가 진실 — folder_id 는 릴레이가 해석 못 할 수 있어 링크 우선.
  const companyShareLink = String(dirRow?.naver_works_company_share_link || "").trim();
  if (!companyShareLink) {
    return {
      ok: false,
      status: 409,
      message: `「${companyName}」 업체의 폴더연결 주소가 없습니다. 관리자에게 업체정보관리에서 마이박스 폴더 주소 등록을 요청해 주세요.`
    };
  }

  const shootOut = await relayCreateMyboxFolder(shootFolderName, companyShareLink);
  if (!shootOut.ok || !shootOut.fileId) {
    return { ok: false, status: 502, message: shootOut.message || "촬영일 폴더 생성 실패" };
  }
  const wantPhoto = needsPhotoFolder(schedule.composition);
  const wantVideo = needsVideoFolder(schedule.composition);
  // 업체명도 폴더 금지문자 제거(하위 폴더명에 들어가므로)
  const compForName = companyFolderName(companyName);
  const made = { shootFolder: shootFolderName, shootFolderReused: Boolean(shootOut.reused), photo: "", video: "" };
  if (wantPhoto || !wantVideo) {
    const nm = `${mmdd}${compForName}사진원본`;
    const x = await relayCreateMyboxFolder(nm, shootOut.fileId);
    if (!x.ok) return { ok: false, status: 502, message: x.message || "사진원본 폴더 생성 실패" };
    made.photo = nm;
  }
  if (wantVideo) {
    const nm = `${mmdd}${compForName}영상원본`;
    const x = await relayCreateMyboxFolder(nm, shootOut.fileId);
    if (!x.ok) return { ok: false, status: 502, message: x.message || "영상원본 폴더 생성 실패" };
    made.video = nm;
  }
  made.companyShareLink = companyShareLink;
...
/** 옛 하위폴더 4종 폐지(2026-09-23) — 고칠 것이 없다. ensureShootCompositionSubfoldersNaverSync 주석 참조 */
function rowNeedsSubfolderHeal(_row, _composition) {
  return false;
}
...
async function mergeShootCompositionSubfoldersIntoRowNaver(headers, schedule, row) {
  ...
  if (changed) {
    // ‼️스테일 스냅샷 덮어쓰기 방지(2026-07-29): 이 heal 은 하위폴더 id 4종을 고치는 경로다.
    //   row 는 잡 시작 시 뜬 스냅샷일 수 있어(크론은 수 분 걸림), 통째로 upsert 하면 그 사이
    //   기록된 납품완료 시각을 null 로 되돌리거나(notify 레이스) revert 된 값을 되살린다.
    //   이 함수가 주인이 아닌 진행상태·이력 컬럼은 payload 에서 항상 뺀다.
    const persistPayload = { ...next };
    for (const k of [
      "photo_notified_at",
      "video_notified_at",
      "photographer_site_done_at",
      ...
      delete persistPayload[k];
    }
    await persistShootDeliverySubfolderMerge(headers, schedule.id, persistPayload);
    return next;
  }
  return next;
}
...
async function completePhotographerShoot({
  ...
  // 디렉터리의 raw customer_phone(업체당 최대 3개, 쉼표 결합)을 우선 사용 → 전부에게 발송.
  // 디렉터리에 없으면 납품 state 캐시(단일) 로 폴백.
  const directoryRawForSite = pickCustomerPhoneRawFromDirectory(
    directoryRows,
    String(schedule.company_name || "").trim(),
    String(schedule.code || "").trim()
  );
  const customerSmsTo =
    directoryRawForSite ||
    String(rowForResponse.customer_phone || row.customer_phone || "");
...
async function listPhotographerShootPanel({ writerLoginId, writerPassword, scheduleIds }) {
  ...
  for (const raw of ids) {
    ...
    // ‼️2026-09-23: 작가 화면 목록은 읽기만 한다(폴더 「복구」 금지).
    //   예전엔 여기서 ensureFolderTreeForScheduleNaver 로 서버 규칙 이름(끝 순번 없음)의 촬영일 폴더를 새로 만들어
    //   진짜 폴더 옆에 빈 중복 폴더(예: 0826백석아이파크101동402호 ↔ …402호1)와 옛 하위폴더 4종이 생겼다.
    //   촬영일 폴더·딥링크는 아침 루틴(scripts/daily_delivery_folders.cjs, 지난 3일 소급)이 만든다.
...
module.exports = {
  ...
  ensureWriterDeliverySubfolders,
