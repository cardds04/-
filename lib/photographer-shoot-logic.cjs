...
async function notifyPhotographerOriginalUploadComplete({ writerLoginId, writerPassword, scheduleId }) {
  ...
  let smsOut = {
    customer_original_upload_notice_sms: "skipped_no_phone",
    smsDeferredQuietHours: false,
    recordAtIso: null,
  };
  try {
    const smsR = await sendPhotographerOriginalUploadSmsToCustomer({
    ...
    smsOut = {
      customer_original_upload_notice_sms: smsR.result,
      smsDeferredQuietHours: !!smsR.deferred,
      recordAtIso: smsR.recordAtIso || null,
    };
  } catch (err) {
  ...
  const { customer_original_upload_notice_sms, smsDeferredQuietHours, recordAtIso: smsRecordAt } = smsOut;

  /** 문자 성공 시 솔라피 시각, 그 외는 처리 완료 시각 — 대시보드 「원」은 솔라피 결과와 무관하게 저장 */
  const doneAt =
    customer_original_upload_notice_sms === "sent" && String(smsRecordAt || "").trim()
      ? String(smsRecordAt).trim()
