...
/** Google Drive API 는 새 폴더 생성을 막지 않아, 운영 플래그로만 제한할 수 있다(명시적 비활성화). */
const DRIVE_FOLDER_CREATE_DISALLOWED_CODE = "DRIVE_FOLDER_CREATE_DISALLOWED";

/**
 * 기본: 새 폴더 생성 허용(Vercel에 변수 없어도 「폴더만들기·납품」이 동작함).
 * 끄려면 DELIVERY_DRIVE_ALLOW_NEW_FOLDERS=0|false|no|off
 */
function isDriveFolderCreateAllowed() {
  const raw = String(process.env.DELIVERY_DRIVE_ALLOW_NEW_FOLDERS ?? "").trim();
  if (!raw) return true;
...
function makeDriveFolderCreateDisallowedError(hint) {
  const err = new Error(
    "DELIVERY_DRIVE_ALLOW_NEW_FOLDERS=0 으로 Drive 새 폴더 생성이 꺼져 있습니다. 폴더가 필요하면 이 값을 제거하거나 1로 바꾼 뒤 재배포하세요."
  );
...
function friendlyDriveFolderCreateDeniedMessage(error) {
  return error?.code === DRIVE_FOLDER_CREATE_DISALLOWED_CODE
    ? "Drive 새 폴더 생성이 환경 변수로 꺼져 있습니다. 배포 설정에서 DELIVERY_DRIVE_ALLOW_NEW_FOLDERS 를 삭제하거나 1 로 두고 재배포해 주세요."
    : null;
}
