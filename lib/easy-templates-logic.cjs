...
function rowToOut(url, r) {
  return {
    ...
    thumb: r.thumb || null,
    preview: Array.isArray(r.preview) ? r.preview : [],   // 미리보기 슬라이드쇼 [{url,dur,kind}]
...
async function upsert(body) {
  ...
  if (Array.isArray(body.preview)) row.preview = body.preview;          // 미리보기 슬라이드쇼 [{url,dur,kind}]
  ...
  if (!r.ok) {
    ...
    if (/thumb|preview/i.test(errText) && ("thumb" in row || "preview" in row)) {
      delete row.thumb; delete row.preview;
