...
function buildPrompt(text, sty, hasRef) {
  if (hasRef) {
...
Place it centered on a perfectly FLAT SOLID WHITE (#FFFFFF) background — no gradient, no background shadow, no scene, no objects. Leave clear margin around the lettering so it can be cut out cleanly.`;
  }
...
Place it centered on a perfectly FLAT SOLID WHITE (#FFFFFF) background — no gradient, no background shadow, no scene, no objects, no extra words besides the given text. Leave clear margin around the lettering so it can be cut out cleanly.`;
}
...
async function generateTitle(body) {
  ...
  // 참조 이미지(관리자가 지정한 스타일) — dataURI 직접 or 스토리지 URL(서버가 받음)
  let refPart = null;
  ...
  if (refImage) {
    ...
    if (m) refPart = { inlineData: { mimeType: m[1], data: m[2] } };
  } else if (refUrl) {
    refPart = await fetchUrlToPart(refUrl);
  }
  const styleKey = String(body?.style || "").trim();
  const sty = TITLE_STYLES[styleKey] || TITLE_STYLES[DEFAULT_STYLE];
  const image = await callGeminiImage(buildPrompt(text, sty, !!refPart), refPart);
  return { ok: true, image, bg: "#ffffff", style: refPart ? "ref" : (TITLE_STYLES[styleKey] ? styleKey : DEFAULT_STYLE), label: refPart ? "📎 참조 스타일" : sty.label, text };
}
