...
function buildPrompt(text, sty, hasRef, transparent) {
  ...
  if (hasRef) {
    // 참조 타이틀을 새 문구로 바꾸되, 글꼴·색감(특히 여러 색/그라데이션)을 그대로 따라가게, 배경은 위 bg 규칙대로
    return `Change the title in this image to this Korean text: "${text}" — keep it perfectly spelled, keep the word spacing.
Match the reference title's lettering closely: same font shape, stroke weight, outline, shadow, and especially the SAME COLOR SCHEME.
If the reference lettering uses multiple colors, a gradient, or a different color per letter or per word, reproduce that exact multi-color look — do NOT simplify it into one single flat color.
${bg}`;
  }
...
async function callOpenAIImage(prompt, ref, transparent) {
  ...
  const quality = String(process.env.OPENAI_IMAGE_QUALITY || "medium").trim();
...
async function generateTitle(body) {
  ...
  // 엔진: 기본 OpenAI(gpt-image 크로마키 → 클라가 키잉해 투명화). TITLE_ENGINE=gemini 로 되돌릴 수 있음.
  const engine = String(body?.engine || process.env.TITLE_ENGINE || "openai").trim().toLowerCase();
  const transparent = String(body?.transparent || process.env.TITLE_TRANSPARENT || "").trim() === "1";
  ...
  if (engine === "openai") {
    const image = await callOpenAIImage(prompt, ref, transparent);
    return { ok: true, image, transparent, bg: transparent ? "transparent" : TITLE_CHROMA_BG, provider: "openai", model: String(process.env.OPENAI_IMAGE_MODEL || "gpt-image-1").trim(), style: styleOut, label: labelOut, text, usedPrompt: prompt };
  }
