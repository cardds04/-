...
function buildPrompt(text, sty, hasRef, transparent) {
  // 배경: 투명(OpenAI 네이티브) 또는 단색 마젠타 크로마(Gemini → 클라가 그 색을 키잉). 글자는 꽉 채워 불투명.
  const bg = transparent
    ...
    : "Place the lettering on a perfectly FLAT SOLID MAGENTA (#FF00FF) background that completely fills the image — one uniform magenta color with NO gradient, NO texture, NO scene (the magenta will be removed to make it transparent). The letters must be SOLID, fully filled, and clearly separated from the magenta background.";
  if (hasRef) {
...
async function callOpenAIImage(prompt, ref) {
  ...
  const model = String(process.env.OPENAI_IMAGE_MODEL || "gpt-image-1-mini").trim();   // 저렴판(투명·한글 OK, ~절반값)
  const size = String(process.env.OPENAI_IMAGE_SIZE || "1024x1024").trim();
  const quality = String(process.env.OPENAI_IMAGE_QUALITY || "medium").trim();   // 비용 제어 (low/medium/high). low는 글자·프롬프트적합도 떨어짐 → 기본 medium(~60원)
  let r;
  if (ref) {
...
async function generateTitle(body) {
  ...
  // 엔진: 기본 Gemini(단색 마젠타 배경 → 클라가 키잉해 투명화). TITLE_ENGINE=openai 면 OpenAI(네이티브 투명).
  const engine = String(body?.engine || process.env.TITLE_ENGINE || "gemini").trim().toLowerCase();
  const transparent = engine === "openai";
  ...
  const bgNote = transparent
    ...
    : `\nPlace the lettering on a perfectly FLAT SOLID MAGENTA (#FF00FF) background filling the whole image — one uniform magenta color, NO gradient, NO texture, NO scene (the magenta will be removed to make it transparent). The letters must be SOLID, fully filled, clearly separated from the magenta.`;
  ...
  if (engine === "openai") {
    const image = await callOpenAIImage(prompt, ref);
    return { ok: true, image, transparent: true, bg: "transparent", provider: "openai", model: String(process.env.OPENAI_IMAGE_MODEL || "gpt-image-1-mini").trim(), style: styleOut, label: labelOut, text, usedPrompt: prompt };
  }
  // Gemini — 마젠타 단색 배경으로 생성, 클라이언트가 bg(#ff00ff) 색을 키잉해 투명 PNG로
  const image = await callGeminiImage(prompt, refPart);
  return { ok: true, image, transparent: false, bg: "#ff00ff", provider: "gemini", model: String(process.env.GEMINI_TITLE_MODEL || "gemini-3.1-flash-image-preview").trim(), style: styleOut, label: labelOut, text, usedPrompt: prompt };
}
