...
async function generateTitle(body) {
  ...
  const image = await callGeminiImage(buildPrompt(text, sty));
  return { ok: true, image, bg: sty.bg, style: TITLE_STYLES[styleKey] ? styleKey : DEFAULT_STYLE, label: sty.label, text };
}
