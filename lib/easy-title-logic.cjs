...
function buildPrompt(text, sty, hasRef, transparent) {
  ...
  if (hasRef) {
    return `Look ONLY at the LETTERING STYLE in the reference image: its typeface/font, stroke weight, FILL color (the color inside the letters), outline, and decorations.
Create a brand-new image containing ONLY this Korean text, perfectly spelled, broken into 1-3 balanced lines: "${text}"
Render it in the EXACT same lettering style as the reference — same font shape, same weight, same SOLID FILL color, same outline. Match the reference as closely as possible (a customer should see the same style).
${bg}
IGNORE the reference's words, photo, people, and any background/scene — reproduce ONLY the lettering style, nothing else. Flat 2D sticker lettering.`;
  }
...
async function callOpenAIImage(prompt, ref) {
  ...
  const quality = String(process.env.OPENAI_IMAGE_QUALITY || "medium").trim();   // 비용 제어 (low/medium/high)
  let r;
  if (ref) {
