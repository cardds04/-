...
async function generateTitle(body) {
  ...
  return { ok: true, image, transparent: true, bg: "transparent", provider: "openai", model: String(process.env.OPENAI_IMAGE_MODEL || "gpt-image-1-mini").trim(), style: styleOut, label: labelOut, text };
}
