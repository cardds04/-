...
async function generateTitle(body) {
  ...
  if (customPrompt) {
    ...
    prompt = buildPrompt(text, sty, !!ref, true);
    styleOut = ref ? "ref" : (TITLE_STYLES[styleKey] ? styleKey : DEFAULT_STYLE);
    labelOut = ref ? "📎 참조 스타일" : sty.label;
  }
