...
async function generateTitle(body) {
  ...
  if (isPortrait) {
    ...
    prompt = `Photorealistic portrait headshot photo of ${text}. A single real person, front-facing, looking straight at the camera, neutral facial expression, both eyes open, face fully visible and well-lit, soft natural studio lighting, plain neutral background, sharp focus on the face, realistic skin texture, high detail. Not a cartoon, not an illustration — a real photograph. No text, no watermark, no border.`;
