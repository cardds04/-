...
async function generateTitle(body) {
  ...
  const isPortrait = !!body?.portrait;   // 🪄 인물 사진 모드 — 얼굴 교체용 사실적 정면 얼굴 사진(gpt-image-2 low, 크로마키 없음)
  const text = String(body?.text || "").trim().slice(0, isPortrait ? 400 : 60);
  if (!text && !isSticker) { const err = new Error(isPortrait ? "바꿀 얼굴을 설명해 주세요." : "타이틀 문구를 입력해 주세요."); err.status = 400; throw err; }
  // 엔진: 기본 OpenAI(gpt-image). TITLE_ENGINE=gemini 로 되돌릴 수 있음.
  // 🏷 스티커는 환경변수와 무관하게 항상 OpenAI gpt-image-2 + low 로 고정(제미나이 안 씀).
  const engine = (isSticker || isPortrait) ? "openai" : String(body?.engine || process.env.TITLE_ENGINE || "openai").trim().toLowerCase();
  const openaiModel = (isSticker || isPortrait) ? "gpt-image-2" : String(process.env.OPENAI_IMAGE_MODEL || "gpt-image-2").trim();
  ...
  if (isPortrait && !body?.quality) quality = "low";   // 얼굴은 기본 low(빠르고 쌈)
  ...
  const transparent = isPortrait ? false : (_tEnv === "1" ? true : _tEnv === "0" ? false : (engine === "openai" && openaiNativeTransparent));   // 얼굴 사진은 실제 배경(크로마키·투명 X)
  ...
  if (isPortrait) {
    // 🪄 얼굴 교체용 — 사실적인 정면 얼굴 사진. 크로마키/투명 없이 자연스러운 배경.
    prompt = `Photorealistic waist-up portrait photo of ${text}. A single real person standing and facing the camera, the FULL HAIRSTYLE clearly visible from top of the head, wearing clearly visible stylish CLOTHING/OUTFIT, the face, the hair, and the outfit must ALL be fully shown in frame, soft natural lighting, plain neutral background, sharp focus, realistic skin and fabric texture, high detail. Not a cartoon, not an illustration — a real photograph. No text, no watermark, no border.`;
    styleOut = "portrait"; labelOut = "🪄 얼굴";
  } else if (isSticker) {
    // 참조사진이 있으면 그 사진 속 '대상'을 스티커로 그려야 함(스타일만 따라하면 안 됨). 글씨가 있으면 대상+글씨를 함께.
    const refS = refPart
      ? "Use the reference image as the SUBJECT. Create ONE sticker that clearly DEPICTS the main subject/character shown in the reference image, redrawn as a clean, bold, stylized illustrated sticker (cut-out style). The subject from the reference MUST be the main artwork of the sticker — never output text alone."
      : "Create ONE cute, eye-catching decorative sticker/badge graphic suitable as a short-form video overlay.";
    const txtS = text
      ? (refPart
          ? `Also add this EXACT Korean text, perfectly spelled (keep word spacing): "${text}", as a small label/banner on the sticker. The subject artwork AND the text must BOTH appear together — the text must NOT replace the artwork.`
          : `The sticker prominently includes this EXACT Korean text, perfectly spelled, keep word spacing: "${text}".`)
      : "No text — just the graphic.";
    prompt = `${refS} ${txtS} A single isolated sticker centered in frame, bold, clean, high-contrast, with crisp edges so it reads clearly over video.${bgNote}`;
