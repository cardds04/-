...
function open(host,{state,save,onClose,main=false,onPlay=()=>{},speak=()=>{},onChange=()=>{},onShop=()=>{},onUse=()=>{}}){
 ...
 const scene=new T.Scene();scene.scale.set(SPACE,1,SPACE);host.dataset.spaceScale=SPACE;scene.background=new T.Color('#e9d5b4');const camera=new T.PerspectiveCamera(38,1,.1,400);camera.position.set(8.3,10.5,12.8);camera.lookAt(0,.3,0);
 ...
 const legacyHouseObjects=scene.children.filter(o=>o.isMesh||o===starter);
 const pet=new T.Group();scene.add(pet);const scale=state.species==='dog'?.9:.85;pet.scale.setScalar(scale*(.9+Math.min((state.day||1)/40,.22)));pet.scale.x/=SPACE;pet.scale.z/=SPACE;pet.position.set(.3,0,1.6);
 ...
 const wingStart=scene.children.length;const houseBlocks=buildHouseWings(scene,()=>running);legacyHouseObjects.push(...scene.children.slice(wingStart));for(const b of houseBlocks){if(b.w<.3||b.d<.3)continue;const cx=b.x>4.5?9:0,cz=b.z<-3.65?-7.5:0;b.x=cx+(b.x-cx)/SPACE;b.z=cz+(b.z-cz)/SPACE;b.w/=SPACE;b.d/=SPACE;}let focusRoom='follow',route=[];const homeLook=pet.position.clone().add(new T.Vector3(0,.6,0));
 if(window.KittyHomeGLB){const binary=Uint8Array.from(atob(window.KittyHomeGLB),c=>c.charCodeAt(0));new GLTFLoader().parse(binary.buffer,'',g=>{if(!running){g.scene.traverse(o=>{o.geometry?.dispose();for(const m of [].concat(o.material||[]))m.dispose();});return;}g.scene.traverse(o=>{if(o.isMesh){o.castShadow=o.receiveShadow=true;for(const m of [].concat(o.material)){if(m.map)m.map.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());}}});scene.add(g.scene);legacyHouseObjects.forEach(o=>o.visible=false);host.dataset.homeModel='blender-v18';},()=>{host.dataset.homeModel='fallback';});}
 camera.position.copy(pet.position).add(new T.Vector3(6.2,8.3,10.2));camera.lookAt(homeLook);
 ...
 const bounds={bed:[1.65/SPACE,1.3/SPACE],quilt:[1.45/SPACE,1.15/SPACE],ball:[.5/SPACE,.5/SPACE],wand:[.9/SPACE,.5/SPACE]};
 for(const g of inventory)if(g.model)bounds[g.id]=[g.width/SPACE,g.width/SPACE];
 for(const id of owned){const g=new T.Group();g.userData.item=id;scene.add(g);const def=inventory.find(a=>a.id===id);let pos=state.roomLayout[id]||defaults[id];if(!pos){outer:for(let z=-2.6;z<3+extra;z+=.65)for(let x=-3;x<3.1;x+=.65)if(valid(id,x,z,0)){pos={x,z,r:0};break outer;}}if(!pos){scene.remove(g);if(!state.storedItems.includes(id))state.storedItems.push(id);continue;}state.roomLayout[id]={...pos};g.position.set(pos.x,0,pos.z);g.rotation.y=pos.r||0;
  ...
  if(!def?.model){g.scale.x/=SPACE;g.scale.z/=SPACE;}placed.set(id,g);
 }
 ...
 renderer.domElement.addEventListener('pointerdown',e=>{const r=renderer.domElement.getBoundingClientRect();pointer.set((e.clientX-r.left)/r.width*2-1,-(e.clientY-r.top)/r.height*2+1);ray.setFromCamera(pointer,camera);if(!decorating&&!fetching){for(const h of ray.intersectObjects(scene.children,true)){let o=h.object;while(o){if(o.userData.homeUse){onUse(o.userData.homeUse);return;}o=o.parent;}if(h.object.name==='floor'||h.object.userData.walkFloor)break;}}const hit=ray.intersectObjects(scene.children.filter(o=>o.name==='floor'||o.userData.walkFloor))[0];if(!hit)return;const x=decorating?Math.round(hit.point.x/SPACE*4)/4:hit.point.x/SPACE,z=decorating?Math.round(hit.point.z/SPACE*4)/4:hit.point.z/SPACE;
  ...
  if(!blocked(x,z)){focusRoom='follow';host.dataset.homeRoom='follow';roomSelect.value='follow';rooms.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.homeRoom==='follow'));walkHome(x,z);wander=99;ring.position.set(x,.045,z);ring.visible=true;}
 });
 ...
 const rooms=document.createElement('nav');rooms.className='home-room-nav';for(const[id,r]of Object.entries(homeRooms)){const b=document.createElement('button');b.textContent=r.name;b.dataset.homeRoom=id;b.onclick=()=>{focusRoom=id;host.dataset.homeRoom=id;roomSelect.value=id;if(!decorating&&id!=='all'&&id!=='follow')walkHome(...(r.entry||[r.x,r.z]));rooms.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.homeRoom===id));wander=id==='follow'?3:99;};rooms.append(b);}const roomSelect=document.createElement('select');roomSelect.className='home-room-select';roomSelect.setAttribute('aria-label','방과 시점 선택');for(const[id,r]of Object.entries(homeRooms)){const o=document.createElement('option');o.value=id;o.textContent=r.name;roomSelect.append(o);}roomSelect.onchange=()=>rooms.querySelector('button[data-home-room='+roomSelect.value+']').click();rooms.append(roomSelect);host.append(rooms);host.dataset.homeRoom='follow';rooms.querySelector('[data-home-room=follow]').classList.add('active');
 ...
 const zoom=wheelZoom(renderer.domElement,host);const travel=pointerTravel(renderer.domElement,host,()=>!decorating&&!fetching);
 const resize=()=>{const r=host.getBoundingClientRect();renderer.setSize(r.width,r.height);camera.aspect=r.width/r.height;camera.zoom=decorating?.95:1.05;camera.updateProjectionMatrix()};const observer=new ResizeObserver(resize);observer.observe(host);resize();
 function animate(t){if(!running)return;if(host.hidden||document.hidden){last=0;raf=requestAnimationFrame(animate);return;}const dt=last?Math.min((t-last)/1000,.1):0;last=t;wander-=dt;
  if(wander<0&&!decorating&&!fetching&&!route.length&&focusRoom!=='all'){for(let i=0;i<20;i++){const x=pet.position.x+(Math.random()-.5)*3,z=pet.position.z+(Math.random()-.5)*3;if(!blocked(x,z)){walkHome(x,z);break}}wander=4+Math.random()*3;}
  const delta=target.clone().sub(pet.position),distance=Math.hypot(delta.x,delta.z),moving=distance>.025&&!decorating;
  if(moving){const step=Math.min(distance,dt*(chasing?2.7:travel.running?3.6:1.65)/SPACE);let nx=pet.position.x+delta.x/distance*step,nz=pet.position.z+delta.z/distance*step;if(blocked(nx,nz)){target.copy(pet.position);route=[];wander=3;nx=pet.position.x;nz=pet.position.z;}heading=Math.atan2(nx-pet.position.x,nz-pet.position.z);pet.position.set(nx,Math.abs(Math.sin(t*.012))*.045,nz);pet.rotation.y+=Math.atan2(Math.sin(heading-pet.rotation.y),Math.cos(heading-pet.rotation.y))*Math.min(1,dt*9);}else{pet.position.y=0;ring.visible=false;if(route.length)target.copy(route.shift());}
  ...
  rig.update(dt,moving,chasing||travel.running);host.dataset.locomotion=moving?(travel.running?'run':'walk'):'idle';host.dataset.petAsset=rig.ready?'ready':'loading';
  legs.forEach((leg,i)=>leg.position.y=.25+(moving?Math.sin(t*.013+(i%2)*Math.PI)*.09:0));tail.rotation.y=Math.sin(t*.009)*.4;head.rotation.z=Math.sin(t*.0014)*.045;const wide=focusRoom==='all',focus=wide?new T.Vector3(4.5,.3,-3.8):decorating&&focusRoom!=='follow'?new T.Vector3(homeRooms[focusRoom].x,.6,homeRooms[focusRoom].z):pet.position.clone().add(new T.Vector3(0,.6,0));focus.x*=SPACE;focus.z*=SPACE;homeLook.lerp(focus,Math.min(1,dt*3));const offset=(wide?new T.Vector3(22,28,32).multiplyScalar(SPACE):new T.Vector3(6.2,8.3,10.2)).multiplyScalar(Math.max(1,.7/camera.aspect)*zoom.distance);camera.position.lerp(focus.clone().add(offset),Math.min(1,dt*3));camera.lookAt(homeLook);host.dataset.cameraX=camera.position.x.toFixed(2);host.dataset.cameraZ=camera.position.z.toFixed(2);host.dataset.petX=pet.position.x.toFixed(1);host.dataset.petZ=pet.position.z.toFixed(1);renderer.render(scene,camera);raf=requestAnimationFrame(animate);
 }
 shutdown=()=>{zoom.dispose();travel.dispose();running=false;cancelAnimationFrame(raf);observer.disconnect();scene.traverse(o=>{o.geometry?.dispose();if(o.material){for(const m of [].concat(o.material))m.dispose();}});renderer.dispose();renderer.forceContextLoss();host.hidden=true;};
