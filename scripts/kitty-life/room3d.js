...
function open(host,{state,save,onClose,main=false,onPlay=()=>{},speak=()=>{},onChange=()=>{},onShop=()=>{},onUse=()=>{}}){
 ...
 const scene=new T.Scene();scene.scale.set(SPACE,1,SPACE);host.dataset.spaceScale=SPACE;scene.background=new T.Color('#e9d5b4');const camera=new T.PerspectiveCamera(38,1,.1,180);camera.position.set(8.3,10.5,12.8);camera.lookAt(0,.3,0);
 ...
 const travel=pointerTravel(renderer.domElement,host,()=>!decorating&&!fetching);
 ...
 function animate(t){if(!running)return;if(host.hidden||document.hidden){last=0;raf=requestAnimationFrame(animate);return;}const dt=last?Math.min((t-last)/1000,.1):0;last=t;wander-=dt;
  ...
  legs.forEach((leg,i)=>leg.position.y=.25+(moving?Math.sin(t*.013+(i%2)*Math.PI)*.09:0));tail.rotation.y=Math.sin(t*.009)*.4;head.rotation.z=Math.sin(t*.0014)*.045;const wide=focusRoom==='all',focus=wide?new T.Vector3(4.5,.3,-3.8):decorating&&focusRoom!=='follow'?new T.Vector3(homeRooms[focusRoom].x,.6,homeRooms[focusRoom].z):pet.position.clone().add(new T.Vector3(0,.6,0));focus.x*=SPACE;focus.z*=SPACE;homeLook.lerp(focus,Math.min(1,dt*3));const offset=(wide?new T.Vector3(22,28,32).multiplyScalar(SPACE):new T.Vector3(6.2,8.3,10.2)).multiplyScalar(Math.max(1,.7/camera.aspect));camera.position.lerp(focus.clone().add(offset),Math.min(1,dt*3));camera.lookAt(homeLook);host.dataset.cameraX=camera.position.x.toFixed(2);host.dataset.cameraZ=camera.position.z.toFixed(2);host.dataset.petX=pet.position.x.toFixed(1);host.dataset.petZ=pet.position.z.toFixed(1);renderer.render(scene,camera);raf=requestAnimationFrame(animate);
 }
 shutdown=()=>{travel.dispose();running=false;cancelAnimationFrame(raf);observer.disconnect();scene.traverse(o=>{o.geometry?.dispose();if(o.material){for(const m of [].concat(o.material))m.dispose();}});renderer.dispose();renderer.forceContextLoss();host.hidden=true;};
