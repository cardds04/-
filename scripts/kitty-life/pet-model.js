...
export function attachPet(parent,state,alive=()=>true){
 ...
 const breed=state.species==='dog'?(state.breed||'corgi'):'cat';
 ...
 if(source)new GLTFLoader().parse(JSON.stringify(source),'',g=>{
  ...
  head=model.getObjectByName('Head');headScale=head?.scale.clone();mixer=new T.AnimationMixer(model);if(breed==='corgi'&&g.animations.length===1&&g.animations[0].name==='clip'){const clip=g.animations[0];g.animations=[T.AnimationUtils.subclip(clip,'Idle',0,30,24),T.AnimationUtils.subclip(clip,'Walk',90,120,24),T.AnimationUtils.subclip(clip,'Run',90,120,24)];}g.animations.filter(c=>['Idle','Walk','Run','Gallop'].includes(c.name)).forEach(c=>actions[c.name]=mixer.clipAction(c));ready=true;parent.userData.assetReady=true;parent.userData.clips=Object.keys(actions);
