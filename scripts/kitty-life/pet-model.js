...
export function attachPet(parent,state,alive=()=>true){
 ...
 const source=window.KittyPetModels?.[state.species==='dog'?(state.breed||'corgi'):'cat'];
 if(source)new GLTFLoader().parse(JSON.stringify(source),'',g=>{
  ...
  model.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;if(state.species==='dog'&&state.breed==='shiba'){o.geometry.deleteAttribute('normal');o.geometry=mergeVertices(o.geometry);o.geometry.computeVertexNormals();}}});
  ...
  head=model.getObjectByName('Head');headScale=head?.scale.clone();mixer=new T.AnimationMixer(model);if(state.breed==='corgi'&&g.animations.length){const clip=g.animations[0];g.animations=[T.AnimationUtils.subclip(clip,'Idle',0,30,24),T.AnimationUtils.subclip(clip,'Walk',90,120,24),T.AnimationUtils.subclip(clip,'Run',90,120,24)];}g.animations.forEach(c=>actions[c.name]=mixer.clipAction(c));ready=true;parent.userData.assetReady=true;parent.userData.clips=Object.keys(actions);
 },error=>{parent.userData.assetError=true;console.warn('Pet model unavailable',error);});
 return {update(dt,moving,fast=false){if(!ready)return;const next=actions[moving?(fast?'Gallop':'Walk'):'Idle']||actions[moving?'Run':'Idle']||Object.values(actions)[0];if(next&&next!==current){current?.fadeOut(.2);next.reset().fadeIn(.2).play();current=next;}mixer.update(dt);if(head&&headScale)head.scale.copy(headScale).multiplyScalar(state.species==='dog'?(state.breed==='walker'?.72:state.breed==='corgi'?1:1.18):1.5);},get ready(){return ready;}};
}
